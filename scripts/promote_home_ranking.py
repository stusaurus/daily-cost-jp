...
def build_block(rows):
    ...
    for row in rows[1:]:
        ...
        rest.append(
            ...
            f'<span class="home-rank-no">{row["rank"]}位</span>'
...
#home-ranking-hero .home-rank-item{{min-width:0;display:grid;grid-template-columns:34px 44px minmax(0,1fr);gap:7px;align-items:center;padding:7px;border:1px solid #ebe4df;border-radius:12px;background:#fff;text-decoration:none;color:#252525}}
...
<section id="home-ranking-hero">
  ...
  <p class="home-rank-lead">日用品に限らず、その時点の総合1位〜10位を毎日自動更新。</p>
  <a class="home-rank-first" href="{first_link}"{first_external}>
...
def main():
    ...
    if not rows:
        print("No ranking rows found; keeping homepage unchanged")
        return
    ...
    print(f"Promoted {len(rows)} realtime ranking items to the top of the homepage")
